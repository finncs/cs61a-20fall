...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        this_label = label(t)
        if this_label == find_value:
            this_label = replace_value
        return tree(this_label, branches(t))
...
def preorder(t):
    ...
    for branch in branches(t):
        preorder_list.extend(preorder(branch))
...
def has_path(t, word):
    ...
    if label(t) != word[0]:
        ...
        if word[1:] == '':
            return True
        return sum([has_path(branch, word[1:]) for branch in branches(t)]) == 1
